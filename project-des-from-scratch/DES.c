...
void plain(char message0[301], int message1[2560], int blocks[40][64], int* nblocks)
{
    ...
    totalLen = len + pad; // 입력글자 수 + 패드 수 = 전체 입력글자
    *nblocks = totalLen / 8;
...
void one_round(const int L[32], const int R[32], const int roundkey2[48], int newL[32], int newR[32], const int Etable[48], const int Ptable[32], const int SBOX[8][4][16])
{
    int f_out[32];
    F(R, roundkey2, f_out, Etable, Ptable, SBOX);
    for (int i = 0; i < 32; i++)
    {
        newL[i] = R[i];
    }
